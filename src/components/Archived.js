...
const Archived = () => {
  ...
  return (
    <div className="archive-page p-4">
      <h2 className="text-2xl font-bold mb-4">Archived Tasks</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {archivedTasks.map((task) => (
          <div key={task.id} className={`p-4 border rounded shadow mb-2 ${task.status === "completed" ? "bg-green-100" : task.status === "failed" ? "bg-red-100" : "bg-yellow-100"} relative`}>
            {/* Task Details */}
